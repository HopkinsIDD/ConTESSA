...
|:---------------------------|:-------------------------|
| **Surveillance**           |                           |
| Percent of all symptomatic infections (not previously quarantining due to contact tracing) that are isolated | `r glue("{round(p$rho_s*100, 1)}", "%")`     |
| Percent of all asymptomatic infections (not previously quarantining due to contact tracing) that are isolated | `r glue("{round(p$rho_a*100, 1)}", "%")`     |
...
```{r detection prob plot, fig.cap = cap, fig.width=6, fig.height=4}
  ...
  pSurv <- ggplot(d, aes(x = prop_detect*100, y = r_effective)) +
