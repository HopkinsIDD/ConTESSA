...
```{r, include=FALSE}
  ...
  pB$mult <- params$mult_b
  pB$rho_a <- params$rho_a_b
  pB$rho_s <- params$rho_s_b
  pB$omega_c <- params$omega_c_b
  pB$omega_h <- params$omega_h_b
  pB$eta <- params$eta_b
  pB$t_p <- params$t_p_b
  pB$t_ps <- params$t_p_b
  pB$t_pa <- params$t_p_b
  pB$t_qh <- params$t_qh_b
  pB$t_qhs <- params$t_qh_b
  pB$t_qha <- params$t_qh_b
  pB$t_qc <- params$t_qc_b
  pB$t_qcs <- params$t_qc_b
  pB$t_qca <- params$t_qc_b
...
## `r glue("Contact Tracing Program Evaluation: ", "{p$name}")`

*Report automatically generated from the [ConTESSA application](https://lucy.shinyapps.io/test-trace-isolate/) on `r Sys.Date()`*
...
|:---------------------------|:-------------------------|:-------------------------|
| **Surveillance**           |                          |                          |
| Percent of all symptomatic infections (not previously quarantined) that are isolated | `r glue("{round(p$rho_s*100, 1)}", "%")`     | `r ifelse(p$rho_s == pB$rho_s, glue("-"), glue("{round(pB$rho_s*100, 1)}", "%"))`  |
| Percent of all asymptomatic infections (not previously quarantined) that are isolated | `r glue("{round(p$rho_a*100, 1)}", "%")`     |  `r ifelse(p$rho_a == pB$rho_a, glue("-"), glue("{round(pB$rho_a*100, 1)}", "%"))`   |
...
```{r surv plot, fig.cap = cap, fig.width=6, fig.height=5}
  ...
  pSurv <- ggplot(d, aes(x = prop_detect, y = r_effective, group = scn_factor, color=scn_factor)) +
      geom_line(size=0.8) +
