...
|:---------------------------|:-------------------------|:-------------------------|
| **Surveillance**           |                          |                          |
| Percent of all symptomatic infections (not previously quarantining due to contact tracing) that are isolated | `r glue("{round(p$rho_s*100, 1)}", "%")`     | `r ifelse(p$rho_s == pB$rho_s, glue("-"), glue("{round(pB$rho_s*100, 1)}", "%"))`  |
| Percent of all asymptomatic infections (not previously quarantining due to contact tracing) that are isolated | `r glue("{round(p$rho_a*100, 1)}", "%")`     |  `r ifelse(p$rho_a == pB$rho_a, glue("-"), glue("{round(pB$rho_a*100, 1)}", "%"))`   |
...
```{r surv plot, fig.cap = cap, fig.width=6, fig.height=5}
  ...
  you_are_here_b <- tibble(
      x = pB$rho_s * (1 - pB$alpha) + pB$rho_a * pB$alpha,
      y = r_eff(pB),
      name = "You are here"
    )
  ...
  pSurv <- ggplot(d, aes(x = prop_detect*100, y = r_effective, group = scn_factor, color=scn_factor)) +
      geom_line(size=0.8) +
      ...
      scale_x_continuous(name = "Percent of new infections isolated (not already quarantining due to contact tracing)") +
      theme_hc()
      
  pSurv
      
  cap <- paste0("**Figure ", fig_counter, "**: Impact of completeness of case isolation, as the percent of cases which were not already in quarantine that are identified by surveillance and isolated, on the reproductive number, assuming that infections are isolated on average ", round(p$t_p, 1), " days after symptom onset in Scenario A. The filled circles represent the reproductive number for Scenarios A and B, described above. The lines represents the reproductive number if the proportion of infections identified and isolated is changed, while all other assumptions for each scenario remain constant. The solid black line represents the reproductive number if there were no contact tracing program, and the dashed black line represents a reproductive number of 1, below which an epidemic would be expected to no longer grow.")
...
```{r hh contact plot, fig.cap=cap, fig.width=6, fig.height=5}
  ...
  pH <- ggplot(d, aes(x = omega_h, y = r_effective, group = scn_factor, color=scn_factor)) +
      geom_line(size=0.8) +
      scale_color_manual(values = c("#4278cf", "#16916c")) +
      geom_point(data=you_are_here, aes(x=x, y=y), pch=21, cex=1.5, stroke=1.25, bg="#ffffff") +
      geom_hline(yintercept=1, lty=2) +
...
```{r c contact plot, fig.cap=cap, fig.width=6, fig.height=5}
  ...
  pC <- ggplot(d, aes(x = omega_c, y = r_effective, group = scn_factor, color=scn_factor)) +
      geom_line(size=0.8) +
      scale_color_manual(values = c("#4278cf", "#16916c")) +
      geom_point(data=you_are_here, aes(x=x, y=y), pch=21, cex=1.5, stroke=1.25, bg="#ffffff") +
      geom_hline(yintercept=1, lty=2) +
  ...
  cap <- paste0("**Figure ", fig_counter, "**: Impact of completeness of community contact tracing, as the proportion of community contacts notified and quarantined through contact tracing, on the reproductive number, assuming that contacts are isolated on average ", round(p$t_qc, 1), " days after symptom onset of the index case. The filled circles represent the reproductive number for Scenarios A and B, described above. The lines represents the reproductive number if the proportion of community contacts notified and quarantined is changed, while all other assumptions for each scenario remain constant. The solid black line represents the reproductive number if there were no contact tracing program, and the dashed black line represents a reproductive number of 1, below which an epidemic would be expected to no longer grow.")
  fig_counter <- fig_counter + 1
